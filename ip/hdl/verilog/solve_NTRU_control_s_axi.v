...
// ==============================================================
`timescale 1ns/1ps
module solve_NTRU_control_s_axi
#(parameter
    ...
    output wire [63:0]                   F_upper,
    output wire [63:0]                   G_upper,
    output wire [63:0]                   f,
    output wire [63:0]                   g,
...
localparam
    ...
    ADDR_F_UPPER_DATA_0 = 7'h18,
    ADDR_F_UPPER_DATA_1 = 7'h1c,
    ADDR_F_UPPER_CTRL   = 7'h20,
    ADDR_G_UPPER_DATA_0 = 7'h24,
    ADDR_G_UPPER_DATA_1 = 7'h28,
    ADDR_G_UPPER_CTRL   = 7'h2c,
    ADDR_F_DATA_0       = 7'h30,
    ADDR_F_DATA_1       = 7'h34,
    ADDR_F_CTRL         = 7'h38,
    ADDR_G_DATA_0       = 7'h3c,
    ADDR_G_DATA_1       = 7'h40,
    ADDR_G_CTRL         = 7'h44,
    ADDR_TMP2_DATA_0    = 7'h48,
    ADDR_TMP2_DATA_1    = 7'h4c,
    ADDR_TMP2_CTRL      = 7'h50,
    WRIDLE              = 2'd0,
    WRDATA              = 2'd1,
    WRRESP              = 2'd2,
    WRRESET             = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_F_upper = 'b0;
    reg  [63:0]                   int_G_upper = 'b0;
    reg  [63:0]                   int_f = 'b0;
    reg  [63:0]                   int_g = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_F_UPPER_DATA_0: begin
                    rdata <= int_F_upper[31:0];
                end
                ADDR_F_UPPER_DATA_1: begin
                    rdata <= int_F_upper[63:32];
                end
                ADDR_G_UPPER_DATA_0: begin
                    rdata <= int_G_upper[31:0];
                end
                ADDR_G_UPPER_DATA_1: begin
                    rdata <= int_G_upper[63:32];
                end
                ADDR_F_DATA_0: begin
                    rdata <= int_f[31:0];
                end
...
assign F_upper           = int_F_upper;
assign G_upper           = int_G_upper;
assign f                 = int_f;
assign g                 = int_g;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_F_UPPER_DATA_0)
            int_F_upper[31:0] <= (WDATA[31:0] & wmask) | (int_F_upper[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_F_UPPER_DATA_1)
            int_F_upper[63:32] <= (WDATA[31:0] & wmask) | (int_F_upper[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_G_UPPER_DATA_0)
            int_G_upper[31:0] <= (WDATA[31:0] & wmask) | (int_G_upper[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_G_UPPER_DATA_1)
            int_G_upper[63:32] <= (WDATA[31:0] & wmask) | (int_G_upper[63:32] & ~wmask);
    end
end
